    ...
    <>
      ...
      <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-auto max-w-[95vw]">
        <nav className="glass-pill rounded-full px-2 py-2 md:px-3 md:py-2 flex items-center gap-2 md:gap-4 shadow-2xl relative">
          ...
          {/* Logo Icon Only - Transparent / Glass Effect */}
          <button 
            onClick={() => onNavigate(Page.HOME)}
            className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:scale-105 transition-transform overflow-hidden p-2 text-white backdrop-blur-md"
          >
          ...
          <div className="relative">
            ...
            {isLangMenuOpen && (
              <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 bg-[#0A0C10] border border-border-subtle rounded-xl overflow-hidden shadow-2xl flex flex-col min-w-[80px]">
                {languages.map((lang) => (
                  <button
                    key={lang}
                    onClick={() => {
                    ...
                    className={`px-4 py-2 text-[10px] font-mono font-bold hover:bg-white/10 text-left transition-colors ${currentLang === lang ? 'text-accent-primary bg-white/5' : 'text-gray-400'}`}
                  >
                    {lang}
                  </button>
                ))}
